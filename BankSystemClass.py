...
class Frontend:
    ...
    def open_system(self):
        print("Therare seven transaction operations:")
        print("1: login")
        print("2: logout")
    ...
    def mode_check(self, transaction_list, valid_account_list, create_acct_list, use_daily_limit):
        ...
        while mode_name != "m" and mode_name != "a":
            ...
            mode_name = input("What kind of a type of session do you want login:")
        print()
        self.check_trans(self.open_system(), mode_name, transaction_list, valid_account_list, create_acct_list, use_daily_limit)
    ...
    def logout(self, transaction_list, valid_account_list):
        if transaction_list.count(1) == 0:  # can not logout before login
            ...
            support = Support()
            support.modify_file_ValidAccount(valid_account_list)
    ...
    def createacct(self, transaction_list, mode, valid_account_list, create_acct_list, use_daily_limit):
        ...
        if transaction_list.count(1) == 0:
            ...
            support = Support()
            if mode == "a":
                account_name = support.get_account_name()
                account_number = support.get_account_number()
                ...
                self.check_trans(self.open_system(), "m", [transaction_list[0]], valid_account_list, create_acct_list, use_daily_limit)
    ...
    def transfer(self, transaction_list, mode, valid_account_list, create_acct_list, use_daily_limit):
        ...
        if transaction_list.count(1) == 0:
            ...
            return 1
        print("Transfer from ------>")
        account_number1 = support.get_account_number()
        print("\nTransfer to -------->")
        account_number2 = support.get_account_number()
...
class Testing:
    def testing(self):
        ...
        support = Support()
        valid_account_list = support.readfile_ValidAccount()
        create_acct_list = []
        use_daily_limit = []
        front = Frontend()
        front.check_trans(front.open_system(), None, [], valid_account_list, create_acct_list, use_daily_limit)
